...
export function montarEscenaSensores(THREE, cuadro, textos) {
  ...
  // quedan ilegibles: se agrandan (kR, con tope por elemento) desde su
  // punto de anclaje, para que no se muevan de donde apuntan.
  let kR = 1;
  function colocar(el, v, ax, ay, op = 1, dy = 0, tope = 1.6) {
    const [x, y] = aPantalla(v);
    const k = Math.min(kR, tope);
    // Si al crecer se sale del cuadro, se recorre hacia adentro.
    let cx = 0, cy = 0;
    if (k > 1) {
      const w = el.offsetWidth * k, h = el.offsetHeight * k, fx = -ax / 100, fy = -ay / 100, M = 8;
      const izq = x - fx * w, der = izq + w, arr = y + dy - fy * h, aba = arr + h;
      cx = Math.max(0, M - izq) - Math.max(0, der - (W - M));
      cy = Math.max(0, M - arr) - Math.max(0, aba - (H - M));
    }
    el.style.transformOrigin = `${-ax}% ${-ay}%`;
    el.style.transform = `translate(${(x + cx).toFixed(1)}px, ${(y + dy + cy).toFixed(1)}px) translate(${ax}%, ${ay}%) scale(${k.toFixed(3)})`;
  ...
  function actualizar(t, transcurrido) {
    ...
    // Paneles proyectados
    colocar($('rotuloLectura'), ANCLA.lectura, 0, -50, suave(0, 0.3, t) * (1 - suave(T.lecturaFin - 0.2, T.lecturaFin + 0.2, t)), 0, 1.8);
    colocar($('plataforma'), ANCLA.plataforma, 0, -50, 1, 0, 1.35);
    // En teléfono los rótulos crecen y el del analista quedaría bajo el aviso:
    // se desvanece cuando entra el aviso, que es el paso siguiente.
    colocar($('analista'), ANCLA.analista, 0, 0, kR > 1 ? a * (1 - nt) : a, (1 - a) * 6, 1.7);
    colocar($('aviso'), ANCLA.aviso, -50, -50, nt, (1 - nt) * 8, 1.6);
